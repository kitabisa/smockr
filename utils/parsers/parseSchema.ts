...
const primitiveTest =
  <T extends 'string' | 'number' | 'integer' | 'boolean' | 'null'>(p: T[]) =>
  (x: JSONSchema7): x is JSONSchema7 & { type: T } =>
    p.includes(x.type as any)

const parsers = [
  {
    test: (x: JSONSchema7): x is JSONSchema7 & { type: 'object' } =>
      x.type === 'object',
    parse: parseObject,
  },
  {
    test: (x: JSONSchema7): x is JSONSchema7 & { type: 'array' } =>
      x.type === 'array',
    parse: parseArray,
  },
  {
    test: (
      x: JSONSchema7,
    ): x is JSONSchema7 & { enum: JSONSchema7Type | JSONSchema7Type[] } =>
      !!x.enum,
    parse: parseEnum,
  },
  {
